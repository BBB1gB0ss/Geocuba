...
import { FiSearch, FiEdit2, FiTrash2, FiUserPlus, FiCheck, FiX } from 'react-icons/fi'
import { motion } from 'framer-motion'
import Button from '../../components/ui/Button'
import Modal from '../../components/ui/Modal'
import Alert from '../../components/ui/Alert'
import Loading from '../../components/ui/Loading'
import { getUsers, updateUser, deleteUser, changeUserRole } from '../../services/userService'
...
const MOCK_USERS = [
  ...
  },
  {
    id: 2,
    name: 'Laura Martínez',
    email: 'laura@geodesa.com',
    role: 'specialist',
    lastLogin: '2023-11-14T14:25:00Z',
    status: 'active'
  },
  {
    id: 3,
    name: 'Ana López',
    email: 'ana@geodesa.com',
    role: 'user',
  ...
  },
  {
    id: 5,
    name: 'María García',
    email: 'maria@geodesa.com',
    role: 'admin',
    lastLogin: '2023-11-10T10:20:00Z',
    status: 'active'
  },
  {
    id: 6,
    name: 'José Pérez',
    email: 'jose@geodesa.com',
    role: 'user',
    lastLogin: '2023-11-05T13:10:00Z',
    status: 'inactive'
  }
]
    ...
    try {
      ...
      setAlert({
        ...
        message: `El rol de "${selectedUser.name}" ha sido actualizado a "${
          ...
          selectedRole === 'specialist' ? 'Especialista' : 'Usuario General'
        }".`
      })
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="p-4">
          <p className="mb-4">
            Cambiar el rol de <span className="font-medium">{selectedUser?.name}</span>
          </p>
          
          <div className="space-y-4 mb-6">
            <div 
              className={`border rounded-md p-3 cursor-pointer ${
                selectedRole === 'admin' ? 'border-primary-500 bg-primary-50' : 'border-gray-300'
              }`}
              onClick={() => setSelectedRole('admin')}
            >
              <div className="flex items-center">
                <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                  selectedRole === 'admin' ? 'border-primary-500' : 'border-gray-300'
                }`}>
                  {selectedRole === 'admin' && <FiCheck className="w-3 h-3 text-primary-500" />}
                </div>
                <div className="ml-3">
              ...
              <p className="text-sm text-gray-500 mt-1 ml-8">
                Acceso completo al sistema. Puede gestionar usuarios y todas las capas.
              </p>
            </div>
            
            <div 
              className={`border rounded-md p-3 cursor-pointer ${
                selectedRole === 'specialist' ? 'border-primary-500 bg-primary-50' : 'border-gray-300'
              }`}
              onClick={() => setSelectedRole('specialist')}
            >
              <div className="flex items-center">
                <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                  selectedRole === 'specialist' ? 'border-primary-500' : 'border-gray-300'
                }`}>
                  {selectedRole === 'specialist' && <FiCheck className="w-3 h-3 text-primary-500" />}
                </div>
                <div className="ml-3">
            ...
            <div 
              className={`border rounded-md p-3 cursor-pointer ${
                selectedRole === 'user' ? 'border-primary-500 bg-primary-50' : 'border-gray-300'
              }`}
            ...
            >
              <div className="flex items-center">
                <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                  ...
                  {selectedRole === 'user' && <FiCheck className="w-3 h-3 text-primary-500" />}
                </div>
                <div className="ml-3">
